...
def mainProg():
    ...
    if (sw1, sw2) == (0x90, 0x0):
        ...
        # [ FF, 82, Key_Structure, Key_Number, 06, Key(6bytes) ]
        ...
        for a in range(0, 16):
            COMMAND = [0xFF, 0x86, 0x00, 0x00, 0x05,
                       0x01, 0x00, a*4, 0x60, 0x00]  # Otentikasi
            # [ FF, 86, 00, 00, 05, 01, 00, Block_Number, Type_Key, 00]
            # Type_Key = 60->A, 61->B
            data, sw1, sw2 = connection.transmit(COMMAND)
            for block in range(a*4, a*4+4):
                if block > 0:
                    list_check = [255, 7, 128, 105,
                                  255, 255, 255, 255, 255, 255]
                    # Read [ FF, B0, 00, Block_Number, Read_Byte ]
                    COMMAND = [0xFF, 0xB0, 0x00]
                    # Read Byte = 16byte-> 10, 4byte->4=04
                    COMMAND.append(block)
                    # Desimal, ingin membaca brapa byte .. nanti jadi 10
                    COMMAND.append(16)
                    data, sw1, sw2 = connection.transmit(COMMAND)
                    # ngefilter data yang mempunyai nilai 0 tidak akan ditampilkan
                    ...
                    if len(data) == 0:
                        ...
                        if set(data) == set(list_check):
                            ...
                            print("sector "+str(a)+", block " + str(block) +
        ...
        # Tidak dapat menulis di block 0 karena tempat UID
        if (int(sys.argv[2]) % 4 == 3 or (int(sys.argv[2]) == 0)):
            ...
            COMMAND = [0xFF, 0x86, 0x00, 0x00, 0x05, 0x01, 0x00,
                       int(sys.argv[2]), 0x60, 0x00]  # Otentikasi
            # [ FF, 86, 00, 00, 05, 01, 00, Block_Number, Type_Key, 00]
            # Type_Key = 60->A, 61->B
            data, sw1, sw2 = connection.transmit(COMMAND)
            if (sw1, sw2) == (0x90, 0x0):
                ...
                x = len(masukan)
                listinput = []
                # Write [FF, D6, 00, Block_Number, Write_Byte]
